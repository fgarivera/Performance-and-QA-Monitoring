...
const NAV = [
  { group: "Monitor", items: [
    { key: "overview", label: "Overview",         icon: window.I.layout,    homeFor: "Manager"    },
    { key: "live",     label: "Live Monitoring",   icon: window.I.activity, homeFor: "Supervisor" },
    { key: "queue",    label: "Review Queue",      icon: window.I.inbox,    homeFor: "QA Analyst", badge: "queueCount" },
    { key: "gating",   label: "AI Response Gating",icon: window.I.zap,                              badge: "gatingPending" },
  ]},
  { group: "Performance", items: [
    { key: "agents",   label: "Agents",          icon: window.I.users   },
    { key: "trends",   label: "Trends & Analytics", icon: window.I.trends },
    { key: "reports",  label: "Reports",         icon: window.I.reports },
  ]},
  { group: "QA Operations", items: [
    { key: "sampling",  label: "QA Sampling",     icon: window.I.shuffle },
    { key: "framework", label: "QA Framework",    icon: window.I.scale   },
    { key: "knowledge", label: "Knowledge Base",  icon: window.I.reports },
    { key: "audit",     label: "Audit Log",       icon: window.I.audit   },
  ]},
  { group: "Settings", items: [
    { key: "integrations", label: "Integrations",        icon: window.I.plug     },
    { key: "alerts",       label: "Alerts & Notifications", icon: window.I.bell  },
    { key: "team",         label: "Team & Permissions",  icon: window.I.shield   },
  ]},
];
...
const TopBar = ({ active, dateRange, setDateRange,
  ...
  return (
    <header className="h-14 bg-white border-b border-slate-200 flex items-center px-5 gap-4 flex-shrink-0 relative z-30">
      ...
      {/* Role switcher */}
      <div className="relative">
        <button onClick={() => setRoleSwitcherOpen(o=>!o)}
                className="flex items-center gap-2 px-2.5 py-1.5 rounded-md border border-slate-200 hover:bg-slate-50">
          <span className="w-2 h-2 rounded-full bg-[var(--accent)]"/>
          <span className="text-xs font-semibold text-slate-700">Viewing as</span>
          <span className="text-xs font-semibold text-slate-900">{role}</span>
        ...
        {roleSwitcherOpen && (
          <div className="absolute right-0 top-full mt-1 w-64 bg-white border border-slate-200 rounded-md shadow-lg z-30 overflow-hidden">
            <div className="px-3 py-2 text-[10px] font-semibold uppercase tracking-wider text-slate-400 border-b border-slate-100">Switch role (demo)</div>
            {[
              { r: "Manager",     home: "Overview Dashboard",  desc: "Reads team health and exports coaching plans." },
              { r: "Supervisor",  home: "Live Monitoring",     desc: "Watches live streams and intervenes on risk." },
              { r: "QA Analyst",  home: "Review Queue",        desc: "Validates / overrides AI scores." },
            ].map(o => (
              <button key={o.r} onClick={() => { setRole(o.r); setRoleSwitcherOpen(false); addToast(`Now viewing as ${o.r}. Home: ${o.home}.`); }}
                      className={`w-full text-left px-3 py-2 hover:bg-slate-50 ${role === o.r ? "bg-[var(--accent-tint)]/40" : ""}`}>
                <div className="flex items-center justify-between">
                  <div className="text-xs font-semibold text-slate-900">{o.r}</div>
                  {role === o.r && <window.I.check className="w-3 h-3 text-[var(--accent)]"/>}
                </div>
