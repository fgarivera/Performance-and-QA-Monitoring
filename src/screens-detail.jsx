...
const ScorecardScreen = ({ agent, openInteraction, addToast, setExportModal }) => {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Coaching Tracker — closes PRD §6 Use Case 3 loop: did coaching happen + impact */}
      <window.Card
        title={<div className="flex items-center gap-2">
          <h3 className="text-base font-semibold text-slate-900">Coaching tracker</h3>
          <window.InfoTip title="Coaching tracker">Records that coaching actually happened, who delivered it, and the measurable QA score impact afterwards. Closes the loop between AI recommendation and outcome — PRD §6 Use Case 3.</window.InfoTip>
        </div>}
        action={<window.Btn kind="ghost" size="sm" icon={<window.I.plus className="w-3 h-3"/>}>Log session</window.Btn>}>

        {/* Impact tiles */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
          {[
            { l:"Sessions completed", v:window.COACHING_IMPACT.completed, sub:`of ${window.COACHING_IMPACT.totalSessions} planned`,         color:"emerald" },
            { l:"Avg QA delta",       v:`+${window.COACHING_IMPACT.avgDelta}`, sub:"pts gained per session",                                color:"violet"  },
            { l:"Success rate",       v:`${window.COACHING_IMPACT.successRate}%`, sub:"plans hit target within 30d",                        color:"sky"     },
            { l:"Upcoming",           v:window.COACHING_IMPACT.scheduled, sub:"scheduled this week",                                        color:"amber"   },
          ].map((k, i) => (
        ...
        {/* Sessions list */}
        <div className="space-y-2">
          {window.COACHING_SESSIONS.map(s => {
            const delta = s.qaAfter != null && s.qaBefore != null ? +(s.qaAfter - s.qaBefore).toFixed(1) : null;
            ...
            return (
              <div key={s.id} className="rounded-lg border border-slate-200 p-3 hover:bg-slate-50/40">
                <div className="flex flex-wrap items-start gap-3">
                  <div className="flex-shrink-0">
                    <div className="text-[10px] font-semibold uppercase tracking-wider text-slate-500">{s.date}</div>
                    <div className="text-[11px] text-slate-700">{s.time}</div>
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-0.5">
                      <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded border ${statusPill}`}>{s.status}</span>
                      {s.duration && <span className="text-[10px] text-slate-500">· {s.duration}</span>}
                      <span className="text-[10px] text-slate-500">· {s.fromPlan}</span>
                    </div>
                    <div className="text-sm font-semibold text-slate-900">{s.topic}</div>
                    <div className="text-[11px] text-slate-500">Coached by <span className="font-medium text-slate-700">{s.supervisor}</span> · Target: {s.target}</div>
                    {s.notes && <div className="text-xs text-slate-600 mt-1.5 italic">"{s.notes}"</div>}
                    {s.nextStep && (
                      <div className="text-[11px] text-[var(--accent-deep)] mt-1.5 inline-flex items-center gap-1">
                        <window.I.chevRight className="w-3 h-3"/>Next: {s.nextStep}
                      </div>
                    )}
                  </div>
                  {/* Impact card */}
                  <div className="flex-shrink-0 w-44 rounded-md border border-slate-200 bg-white p-2">
                    <div className="text-[10px] font-semibold uppercase tracking-wider text-slate-500 mb-0.5">QA score impact</div>
                    {delta != null ? (
                      <>
                        <div className="flex items-baseline gap-1.5">
                          <span className="text-xs tabular-nums text-slate-500">{s.qaBefore}</span>
                          <window.I.chevRight className="w-3 h-3 text-slate-300"/>
                          <span className={`text-base font-semibold tabular-nums ${isPositive ? "text-emerald-700" : "text-rose-700"}`}>{s.qaAfter}</span>
                          <span className={`text-[11px] font-semibold ${isPositive ? "text-emerald-700" : "text-rose-700"}`}>
                            {isPositive ? "+" : ""}{delta}
                          </span>
                        </div>
                        <div className={`text-[10px] mt-0.5 ${isPositive ? "text-emerald-700" : "text-rose-700"}`}>
                          {isPositive ? "Target met" : "Below target"}
                        </div>
                      </>
                    ) : (
                      <div className="text-[11px] text-slate-500 italic">Not yet measured</div>
                    )}
                  </div>
                </div>
              </div>
