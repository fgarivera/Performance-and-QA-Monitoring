    ...
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {heroStats.map((k, i) => (
            <window.Stat key={i} {...k}
              labelExtra={
                k.label === "QA coverage" ? <window.InfoTip title="QA coverage">% of customer interactions evaluated by AI. Manual QA covers 1–5% with sampling; this product covers 100%.</window.InfoTip> :
                k.label === "Flag rate"   ? <window.InfoTip title="Flag rate">% of interactions flagged for any risk — compliance, sentiment, low confidence, SOP deviation, or quality. Each flag has a reason; see Review Queue.</window.InfoTip> :
                null
              }/>
          ))}
        </div>
...
const QueueScreen = ({ openInteraction, queueRows, addToast, channels }) => {
  ...
  const reasonChip = (key) => {
    const map = {
      ...
      dataquality: "bg-teal-50 text-teal-700 border-teal-200",
      failure:     "bg-fuchsia-50 text-fuchsia-700 border-fuchsia-200",
    };
      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-wrap items-center gap-2 px-3 py-2.5 border-b border-slate-100">
          {window.TRIGGER_REASONS.map(r => {
            ...
            const tip = ({
              compliance:  "Required disclosure missing, regulated language used incorrectly, or data-handling rule broken. PRD §7.2.",
              sentiment:   "Customer frustration signals — repeated negative phrases, escalation keywords, sustained negative sentiment.",
              confidence:  "AI's own confidence in the score was below the cutoff (default 65%). Sent here so a human can decide.",
              sop:         "Agent deviated from the script or expected process flow for this scenario.",
              vip:         "VIP customer or one who explicitly requested escalation / a supervisor.",
              dataquality: "AI couldn't reliably evaluate — partial transcript, low-confidence STT, missing customer ID, etc.",
              failure:     "AI eval failed entirely — timeout, unsupported interaction type, or rate-limited agent. Needs human triage.",
            })[r.key];
            return (
              <button key={r.key} onClick={()=>setFilter(r.key)}
                ...
                {tip && <window.InfoTip title={r.label}>{tip}</window.InfoTip>}
              </button>
            );
          })}
              ...
              <tbody>
                {filtered.map(r => (
                  <tr key={r.id} className="border-b border-slate-100 hover:bg-slate-50 cursor-pointer group" onClick={()=>openInteraction({...r}, "queue")}>
                    ...
                    <td className="px-2 py-2">
                      {r.score == null
                        ? <span className="text-xs text-slate-400" title="AI did not score this interaction">—</span>
                        : <window.ScorePill value={r.score}/>}
                    </td>
                    <td className="px-2 py-2">
                      {r.confidence == null ? (
                        <span className="text-[11px] text-slate-400" title="No evaluation completed">—</span>
                      ) : (
                        <div className="flex items-center gap-1.5">
